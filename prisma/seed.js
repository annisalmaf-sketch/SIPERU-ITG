...
async function main() {
  ...
  await prisma.user.upsert({
    where: { email: "kajur2@itg.ac.id" },
    update: {},
    create: {
      email: "kajur2@itg.ac.id",
      name: "Dr. Eng. Andi R.",
      password: "password123",
      role: "KAJUR",
    },
  ...
  await prisma.user.upsert({
    where: { email: "kajur3@itg.ac.id" },
    update: {},
    create: {
      email: "kajur3@itg.ac.id",
      name: "Prof. Dr. Siti M.",
      password: "password123",
      role: "KAJUR",
    },
  ...
  for (const mhs of students) {
    ...
    await prisma.user.upsert({
      where: { email },
      update: { password: "itg@garut", name: mhs.name },
      create: {
        id: `mhs-${mhs.nim}`,
        email,
        name: mhs.name,
