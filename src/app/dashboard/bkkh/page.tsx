...
export default async function BKKHDashboard() {
  const session = await getServerSession(authOptions);

  if (!session || ((session.user as any).role !== "BKKH" && (session.user as any).role !== "ADMIN")) {
    redirect("/login");
  }
  ...
  const kajurLevel = 1;
  const targetPendingStatus = "PENDING_1";
      ...
      <div className="flex flex-col gap-12 animate-fade-in pb-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6">
          <div className="space-y-1">
            <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight italic">Persetujuan BKKH</h1>
            <p className="text-slate-500 dark:text-slate-400 font-medium">Tinjau dan validasi permintaan peminjaman ruangan tahap 1.</p>
          </div>
        ...
        <PendingNotificationBanner count={pendingCount} role="BKKH" />
