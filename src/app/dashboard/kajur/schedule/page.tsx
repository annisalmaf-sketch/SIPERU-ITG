...
import { getClassSchedules, createClassSchedule, deleteClassSchedule } from "@/actions/schedule";
...
export default function ScheduleManagementPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      setSuccess("Jadwal perkuliahan berhasil ditambahkan!");
      setShowForm(false);
      setFormData({
        roomId: "",
        dayOfWeek: "1",
      ...
      setError(res.error || "Gagal menambahkan jadwal");
    }
  ...
  const filteredSchedules = schedules.filter(s => {
    ...
    const matchesDay = dayFilter === "all" || s.dayOfWeek === parseInt(dayFilter);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mt-1">
            Kelola jadwal perkuliahan tetap yang memblokir ruangan di kalender utama.
          </p>
        </div>
        ...
        >
          {showForm ? "Batal" : <><Plus size={18} /> Tambah Jadwal</>}
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4 border-b dark:border-slate-800 pb-2">Form Tambah Jadwal</h2>
          <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Mata Kuliah</label>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Dosen Pengampu</label>
              <div className="relative">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Pilih Ruangan</label>
              <select 
                required
              ...
              >
                ...
                {rooms.filter(r => r.type === "CLASSROOM" || r.type === "LABORATORY").map(room => (
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Waktu Mulai</label>
              <input 
                type="time" 
                required
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Waktu Selesai</label>
              <input 
                type="time" 
                required
            ...
            <div className="md:col-span-2 pt-2 border-t dark:border-slate-800">
              ...
              >
                {isSubmitting ? <Loader2 size={18} className="animate-spin" /> : "Simpan Jadwal"}
              </button>
            </div>
          </form>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-slate-900 p-12 rounded-2xl border border-slate-200 dark:border-slate-800 text-center">
          <CalendarClock size={48} className="mx-auto text-slate-300 dark:text-slate-600 mb-4" />
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">Tidak ada jadwal</h3>
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Belum ada jadwal perkuliahan yang ditambahkan.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredSchedules.map((schedule) => (
            <div key={schedule.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
              ...
              <h3 className="font-bold text-slate-900 dark:text-white text-lg leading-tight mb-1">{schedule.courseName}</h3>
